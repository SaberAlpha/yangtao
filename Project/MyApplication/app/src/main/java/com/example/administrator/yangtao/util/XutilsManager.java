package com.example.administrator.yangtao.util;
...
public class XutilsManager {
    ...
    private void initDbManager() {
        File dbfile=new File("/sdcard/YTPxutils/db");
        if(!dbfile.exists()){
            dbfile.mkdir();
        }

        DbManager.DaoConfig config=new DbManager.DaoConfig().setDbDir(dbfile)
                .setDbName("YangTaoXutils").setDbVersion(1).setDbUpgradeListener(new DbManager.DbUpgradeListener() {
                    @Override
